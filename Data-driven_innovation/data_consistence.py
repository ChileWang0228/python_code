...
consistent_file_dir = 'consistent/'  # 一致性csv文件存放地址
data_to_train_and_test = 'data_to_train_and_test/'  # 生成的训练集和测试集csv文件存放地址
...
def build_consistent_csv():
    ...
    new_columns_name = pd.read_csv(csv_file_name[0]).drop(drop_columns_list[0], axis=1).columns.values.tolist()
    for i in range(len(csv_file_name)):
        pd_data = pd.read_csv(csv_file_name[i])
        ...
        # with open(json_file_name[i], 'r', encoding='UTF-8') as fr:  # 读取对应特征的字典解释
        #     important_feature_dict = dict(json.load(fr))
        # with open('feature.txt', 'a', encoding='UTF-8') as fw:
        #     for col in columns:
        #         fw.write(str(col) + ": " + str(important_feature_dict[col] + ',' + '\t\t\t\t\t\t\t\t\t'))
        #     fw.write('\n')
...
def build_train_and_test_set():
    ...
    取10~12年的数据做训练集：
        数值型特征选择其变化率;
        标称型特征选择当年最新的;
        标签取2014年fid与2012年fid的交集，存在的交集说明fid还在,fid中的pid设置标签为１：仍然属于该fid，并未分家；
        否则标签设置成0。
    取12~14年的数据集做验证集：
        数值型和标称型特征做法同上；
        标签取2016与2014年的fid交集，做法同上。
    最后取16年的数据集做测试集合：
        数值型和标称型特征做法同上。
    :return:生成训练集和验证集合与测试集的CSV文件
    """
    ...
    # 若2012年的fid在交集中，　则说明家庭并未流失，pid仍然属于原先的fid,label赋值为０
    ...
    new_data_2010_fid = new_data_2010['fid']
    train_set = new_data_2012[new_data_2012.fid.isin(new_data_2010_fid)].copy()
    train_set.to_csv(data_to_train_and_test + 'train_set.csv', index=False)  # 不要索引

    # 构造验证集合
    new_data_2012_fid = new_data_2012['fid']
    valid_set = new_data_2014[new_data_2014.fid.isin(new_data_2012_fid)].copy()
    valid_set.to_csv(data_to_train_and_test + 'valid_set.csv', index=False)
